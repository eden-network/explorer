...
export default function Pagination({ numPages, perPage, total, activePage, setPage }: { numPages: number, perPage: number, total: number, activePage: number, setPage: (page: number) => any }) {
  ...
  return (<div className="px-4 py-3 flex items-center justify-between sm:px-6">
    <div className="flex-1 flex justify-between sm:hidden">
      <a
        onClick={previous}
        className="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 cursor-pointer select-none"
      >
        Previous
      </a>
      <a
        onClick={next}
        className="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 cursor-pointer select-none"
      >
        Next
      </a>
    </div>
